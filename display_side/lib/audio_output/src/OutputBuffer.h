...
#include <Arduino.h>
#include <freertos/FreeRTOS.h>
#include <freertos/semphr.h>  // Ensure Semaphore functions are available
...
public:
  OutputBuffer(int number_samples_to_buffer)
    ...
    // Create semaphore and check if successful
    m_semaphore = xSemaphoreCreateBinary();
    if (!m_semaphore) {
      Serial.println("Failed to create semaphore");
      free(m_buffer);  // Free buffer if semaphore fails
      return;
    }
    xSemaphoreGive(m_semaphore);
  }
  ...
  void add_samples(const uint8_t *samples, int count) {
    if (!m_buffer || !m_semaphore) return;  // Prevent access if uninitialized
    ...
    m_available_samples = min(m_available_samples + count, m_buffer_size); // Prevent overflow
    xSemaphoreGive(m_semaphore);
  }
  ...
  void remove_samples(int16_t *samples, int count) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (m_buffering && m_available_samples < m_number_samples_to_buffer) {
        ...
        m_read_head = (m_read_head + 1) % m_buffer_size;
        m_available_samples = max(0, m_available_samples - 1); // Prevent underflow
      }
    }
    xSemaphoreGive(m_semaphore);
  }
